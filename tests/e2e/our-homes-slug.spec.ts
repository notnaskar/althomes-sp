...
test.describe('Property detail page (/our-homes/[slug])', () => {
  ...
  test('renders specs strip (guests, bedrooms, bathrooms)', async ({ page }) => {
    await page.goto(propertyUrl)
    const specsStrip = page.locator('[class*="border-b"]').filter({ hasText: /Guests|Bedrooms|Bathrooms/i })
    await expect(specsStrip).toBeVisible()
  })

  test('renders location section', async ({ page }) => {
    await page.goto(propertyUrl)
    const locationSection = page.locator('section').filter({ hasText: /View on Google Maps|location/i })
  ...
  test('renders bottom CTA with FIND AVAILABILITY button', async ({ page }) => {
    await page.goto(propertyUrl)
    await expect(page.getByRole('link', { name: 'FIND AVAILABILITY' })).toBeVisible()
  })
  ...
  test('renders reviews section when published reviews exist', async ({ page }) => {
    await page.goto(propertyUrl)
    // Reviews section is conditional — just verify page loads without error
    await expect(page.locator('main')).toBeVisible()
  })

  test('renders experiences section when experiences are linked', async ({ page }) => {
    await page.goto(propertyUrl)
